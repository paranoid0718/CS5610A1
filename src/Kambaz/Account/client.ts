...
export const findAllCourses = async () => {
    const {data} = await axiosWithCredentials.get(`${HTTP_SERVER}/api/courses`);
    return data;
}
...
export const enrollUserInCourse = async (courseId: string) => {
  ...
  const url = `${HTTP_SERVER}/api/users/current/courses/enroll`;
...
export const unenrollUserFromCourse = async (courseId: string) => {
  const url = `${HTTP_SERVER}/api/users/current/courses/unenroll`;
