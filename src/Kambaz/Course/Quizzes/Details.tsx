    ...
    <div>
      ...
      >
        <Row className="gx-5 gy-2">
          ...
          <Col sm={5} className="text-end fw-semibold text-muted">Assignment Group</Col>
          <Col sm={7}>{(quiz as any).assignmentGroup ?? "QUIZZES"}</Col>

          <Col sm={5} className="text-end fw-semibold text-muted">Shuffle Answers</Col>
          <Col sm={7}>{quiz.shuffleAnswers ? "Yes" : "No"}</Col>

          <Col sm={5} className="text-end fw-semibold text-muted">Time Limit</Col>
          <Col sm={7}>{quiz.timeLimit != null ? `${quiz.timeLimit} Minutes` : "—"}</Col>

          <Col sm={5} className="text-end fw-semibold text-muted">Multiple Attempts</Col>
          <Col sm={7}>{quiz.multipleAttempts ? "Yes" : "No"}</Col>

          <Col sm={5} className="text-end fw-semibold text-muted">Show Correct Answers</Col>
          <Col sm={7}>{quiz.showCorrectAnswers ? "At release time" : "Immediately"}</Col>

          <Col sm={5} className="text-end fw-semibold text-muted">One Question at a Time</Col>
          <Col sm={7}>{quiz.oneQuestionAtATime ? "Yes" : "No"}</Col>

          <Col sm={5} className="text-end fw-semibold text-muted">Webcam Required</Col>
